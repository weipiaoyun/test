...
sub updateWeek {
	...
	while ($do_lay <= $#lay) {
		...
		if (grep /^$h->{weekGroup}$/, @{$h->{weekGroupOk}}) {
			$do_lay++;
			next;
		}
		...
		$jobTest =~ s/(\D{2,3}\d{5,6}\D\d(-(en|j))?).*/$1/;
		$h->{weekSide} = undef;
		$h->{weekMode} = undef;
		$h->{weekLayer} = undef;
		...
		$h->{week}{value}{$lay[$do_lay]} =  "";
			...
			#阻焊
			my $type = ${$h->{filmtype}}[$do_lay];
			if (${$h->{filmtype}}[$do_lay] =~ /阻焊/) {
				$type = "防焊";
			}

			my $week = $h->getWeekModeByLayerType1(${$h->{filmtype}}[$do_lay], $jobTest);
			...
			if ($week =~ /WWYY|YYWW/i) {
				...
				push @{$h->{weekGroupOk}}, $h->{weekGroup};
				...
				#$h->UpdateFilmStatus();
				#print "########### $h->{weekMode} ne $h->{weekMode4Job} or $h->{week4Job} ne $h->{week4Apply} ##########\n";
				$h->UpdateWeekStatus();
			}
...
sub getLayerCheck {
	...
	#签名捞取数据库。数据库捞取不到捞取料号里面的。
	print "$h->{Job} $layer $checkStatus  #### $h->{checkStatus} ####\n";
	my $check = $h->GetLayerStatus1("$h->{Job}", "$layer");
	if (defined $check && $check  ne 'NULL') {
		print "NNNN\n";
		$h->{checkStatus} = "ok";
		return;
	}
